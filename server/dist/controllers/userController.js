...
const registerUser = (ctx, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const requestBody = ctx.request.body;
        const existingEmail = yield model.emailExists(requestBody.email);
        if (existingEmail) {
            ctx.status = 400;
            ctx.body = { message: 'Email already in use' };
        }
        else {
            const createNewUser = yield model.createUser(requestBody);
            ctx.status = 201;
